...
export function nativeReading(id: string, parts: FixturePart[]): TsShardReading {
    ...
    const columns = words.map((word) => ({
        id: word.id + 100,
        role: 'letter' as const,
        text: word.text,
        source_character_ids: [word.id],
        source_unit_ids: [word.id],
        slot_ids: [word.id],
        tier: 'main' as const,
        attached_to_column_id: null,
        status: 'present' as const,
        variant_id: null,
        variant_choice: null,
        anchor_unit_id: null,
        side: null,
        owned_sound_ids: [word.id],
        presented_sound_ids: [],
        rule_occurrence_ids: [],
        silence: null,
    }));
    return {
        ...
        parts: parts.map((part, index) => ({
            ref: part.ref,
            t: [part.start, part.end],
            word_ids: [index],
        })),
        analysis: {
            schema_version: 2,
            result: {
                ref: parts.map((part) => part.ref).join('-'),
                words,
                sounds,
                boundaries: words.map((word) => ({
                    id: word.after_boundary_id,
                    before: word.id,
                    after: word.id + 1 < words.length ? word.id + 1 : null,
                    state: word.id + 1 < words.length ? 'join' : 'stop',
                    stop_sign: null,
                })),
                rule_occurrences: [],
            },
        } as TsShardReading['analysis'],
        source: {
            schema_version: 2,
            source: {
        ...
        timing: {
            ...
            units: parts.map((part, index) => ({
                source_unit_id: index, start_ms: part.start, end_ms: part.end,
            })),
            boundaries: [],
        },
    };
