 ...
 * By default trailing post-completion segments are trimmed (never cut
 * mid-audio) so the clip is one clean take; pass `keepAllTakes` to retain the
 * whole occasion verbatim, which surfaces consecutive full re-recitations as
 * inline repeats (the Timestamps faithfulness view). The returned
 * `[startMs, endMs]` span covers every kept segment AND every word/letter time.
 * A word/letter can bleed a few ms past its segment's `t` bound, so the clip
 * must reach the furthest word/letter end or it would truncate the final word's
 * tail (mirrors the backend `_canonical_verse`).
 */
export function canonicalClip(
    ...
    const kept = opts?.keepAllTakes
        ? occasion
        : comp !== null ? occasion.slice(0, comp + 1) : occasion;
