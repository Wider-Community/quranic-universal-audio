...
<div class="player" class:has-reciter={$playerContext.reciter !== null} class:collapsed={isCollapsed}>
    <button
        type="button"
        class="collapse-tab"
    ...
    {:else}
        <!-- Collapsed mini-row: tap anywhere to expand; the play toggle stays
             live so playback can be driven without re-expanding. -->
        <div
            class="collapsed-row"
            role="button"
            tabindex="0"
...
<style>
    ...
    /* Mobile player: stack the 3-zone row into one centered column. */
    @media (max-width: 767px) {
        ...
        /* Each zone fills the column and centres its items */
        .zone {
            width: 100%;
            justify-content: center;
            flex-wrap: wrap;
