<script lang="ts">
     ...
     * EditOverlay can delegate via `{:else if $editMode === 'delete'}`, and
     * future UI can replace the `confirm()` call with an inline banner or
     * modal without re-threading the entry path.
     *
     * Note: `setEdit('delete', uid)` is called AFTER the confirm() dialog
     * passes — so the store only transitions to 'delete' for the brief instant
     * between confirm and clearEdit(). The panel may mount and immediately
     * unmount in the same tick; that's fine.
     *
     ...
     * condition).
     *
     * audioElRef prop: reserved for future Svelte-native confirm UI.
     */
