...
describe.skipIf(!registry)('registry behavior — parametrized', () => {
  for (const cat of ALL_CATEGORIES) {
    it(`Ignore button visible iff registry.canIgnore (${cat})`, xfail('phase-1', () => {
    ...
    it(`auto_suppress flag is read from registry (${cat})`, xfail('phase-1', () => {
      const row = registry[cat];
    ...
    it(`card type dispatched from registry (${cat})`, xfail('phase-1', () => {
      const row = registry[cat];
      expect(['generic', 'missingVerses', 'missingWords', 'error']).toContain(row.cardType);
    }));
  }

  it('accordion order matches registry', xfail('phase-1', () => {
