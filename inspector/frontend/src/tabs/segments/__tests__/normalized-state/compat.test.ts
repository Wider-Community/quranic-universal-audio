...
describe.skipIf(!chapterStore)('compat shape', () => {
  it('$segData has same shape as before refactor', xfail('phase-4', () => {
  ...
  it('$segAllData has same shape (no _byChapter / _byChapterIndex exposed)', xfail('phase-4', () => {
    if (!segmentsStore) throw new Error('phase-4: stores/segments not yet present');
    let snapshot: any;
  ...
  it('existing components subscribe without modification', xfail('phase-4', () => {
    if (!segmentsStore) throw new Error('phase-4: stores/segments not yet present');
    expect(typeof chapterStore.segData.subscribe).toBe('function');
