 ...
 *  TWO cells (haraka + a standalone mini-meem). The mini-meem displays the
 *  low-meem GLYPH always (cellGlyph normalises it), but its SLOT + calibration
 *  follow the source haraka: ḍamma/fatḥa (MEEM_HI source, 6e2) sits ABOVE,
 *  kasra (MEEM_LO source, 6ed) below — its own calibration since the meem ink
 *  sits differently from a haraka. */
const BY_CODEPOINT: Record<string, HarakaRender> = {
    ...
    '6e2': { scale: 0.8, shiftEm: -0.035, raiseEm: -0.275 }, // mini-meem above slot (iqlab ḍamma/fatḥa source, U+06E2)
    '6ed': { scale: 0.81, shiftEm: -0.09, raiseEm: -0.212 }, // mini-meem below slot (iqlab kasra source, U+06ED)
};
