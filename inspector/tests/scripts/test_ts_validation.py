...
def test_flags_verse_failing_under_tighter_beam():
    ...
    doc = build_ts_validation(_chapters(), results_by_beam, beams, reciter="x", method="kalpy")

    assert doc["verses"] == {"1:2": {"failed_beams": [20], "min_passing_beam": 50}}
...
def test_verse_failing_every_beam_has_no_min_passing():
    ...
    doc = build_ts_validation(_chapters(), results_by_beam, beams, reciter="x", method="kalpy")
    assert doc["verses"] == {"1:2": {"failed_beams": [50, 20], "min_passing_beam": None}}
