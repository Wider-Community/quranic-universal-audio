...
def test_force_set_state_allowed_pair_happy_path(signed_in_client, monkeypatch):
    _stub_state_persist(monkeypatch)
    _replace_state([_row("test_slug")])  # under_review
    client, _ = signed_in_client(role="maintainer")

    res = client.post(
        "/api/admin/state/force-set/test_slug",
        data=json.dumps({
            "to_state": "awaiting_review",
            "reason": "Alternative path to force-release for testing.",
        }),
        headers=_HEADERS,
    )
    assert res.status_code == 200, res.get_json()
    assert res.get_json()["row"]["state"] == "awaiting_review"


def test_force_set_state_disallowed_pair_returns_400(signed_in_client, monkeypatch):
    _stub_state_persist(monkeypatch)
    _replace_state([_row("test_slug", state="awaiting_review", assignee_hf_id=None)])
    client, _ = signed_in_client(role="maintainer")
    ...
    res = client.post(
        ...
        data=json.dumps({
            "to_state": "completed",
            "reason": "Attempting disallowed transition.",
        }),
        headers=_HEADERS,
    )
    assert res.status_code == 400
...
def test_force_set_state_unknown_to_state_returns_400(signed_in_client, monkeypatch):
    _stub_state_persist(monkeypatch)
    _replace_state([_row("test_slug")])
    client, _ = signed_in_client(role="maintainer")
    ...
    res = client.post(
        ...
        data=json.dumps({
            ...
            "reason": "Should never reach state machine.",
        }),
        headers=_HEADERS,
    )
    assert res.status_code == 400
    body = res.get_json()
    assert "unknown to_state" in body["error"]


def test_force_set_state_short_reason_returns_400(signed_in_client, monkeypatch):
    _stub_state_persist(monkeypatch)
    _replace_state([_row("test_slug")])
    client, _ = signed_in_client(role="maintainer")
    ...
    res = client.post(
        "/api/admin/state/force-set/test_slug",
        data=json.dumps({"to_state": "awaiting_review", "reason": "short"}),
        headers=_HEADERS,
    )
    assert res.status_code == 400


def test_force_set_state_by_contributor_returns_403(signed_in_client):
    client, _ = signed_in_client(role="contributor")
    res = client.post(
        "/api/admin/state/force-set/test_slug",
        data=json.dumps({
            "to_state": "awaiting_review",
            "reason": "Should be blocked by RBAC.",
        }),
        headers=_HEADERS,
    )
    assert res.status_code == 403
