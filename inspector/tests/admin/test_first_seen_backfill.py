...
def _first_seen(uid: str) -> str | None:
    row = (
        db.get_conn()
        .execute("SELECT first_seen FROM users WHERE hf_user_id = ?", (uid,))
        .fetchone()
    )
...
def test_pulls_back_to_earliest_transition():
    with db.transaction() as c:
        ...
        c.execute(
            "INSERT INTO transitions(id,ts,event,actor_id) VALUES('t1','2025-01-02T00:00:00Z','x','u1')"
        )
        c.execute(
...
def test_min_across_sources_request_wins():
    with db.transaction() as c:
        ...
        c.execute(
            "INSERT INTO requests(id,kind,requester_id,submitted_at,status) VALUES('r1','k','u2','2025-03-01T00:00:00Z','pending')"
        )
        c.execute(
            "INSERT INTO transitions(id,ts,event,actor_id) VALUES('t3','2025-09-01T00:00:00Z','x','u2')"
        )
    _backfill()
...
def test_idempotent_and_only_backwards():
    with db.transaction() as c:
        ...
        c.execute(
            "INSERT INTO transitions(id,ts,event,actor_id) VALUES('t4','2024-01-01T00:00:00Z','x','u4')"
        )
    _backfill()
    once = _first_seen("u4")
