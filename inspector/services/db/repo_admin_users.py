...
def requests_count_by_user() -> dict[str, int]:
    rows = (
        get_conn()
        .execute("SELECT requester_id, COUNT(*) AS n FROM requests GROUP BY requester_id")
        .fetchall()
    )
...
def lifetime_claims(hf_user_id: str) -> int:
    row = (
        get_conn()
        .execute("SELECT COUNT(*) FROM claims WHERE assignee_id = ?", (hf_user_id,))
        .fetchone()
    )
    return int(row[0]) if row else 0
...
def last_activity(hf_user_id: str) -> str | None:
    row = (
        get_conn()
        .execute("SELECT MAX(ts) FROM transitions WHERE actor_id = ?", (hf_user_id,))
        .fetchone()
    )
...
def requests_by_status(hf_user_id: str) -> dict[str, int]:
    rows = (
        get_conn()
        .execute(
            "SELECT status, COUNT(*) AS n FROM requests WHERE requester_id = ? GROUP BY status",
            (hf_user_id,),
        )
        .fetchall()
    )
