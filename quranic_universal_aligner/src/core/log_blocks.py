...
def build_timing(profiling, cpu_stats: Optional[dict], worker_dispatch: Optional[dict],
    ...
    Args:
        ...
        estimate_given_s: Value returned by `estimate_duration()` at request entry, or None.
        device: "gpu" / "cpu".
    """
...
def build_segments(seg_infos, debug_collector,
                   ...
                   all_special_refs: set, include_dp_debug: bool = True) -> list[dict]:
...
def build_results_summary(segments: list, profiling,
    ...
    return {
        ...
        "tier1_attempts": int(getattr(profiling, "tier1_attempts", 0) or 0),
        "tier1_passed":   int(getattr(profiling, "tier1_passed", 0) or 0),
        "tier2_attempts": int(getattr(profiling, "tier2_attempts", 0) or 0),
        "tier2_passed":   int(getattr(profiling, "tier2_passed", 0) or 0),
