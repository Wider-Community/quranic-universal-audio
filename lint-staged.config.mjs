    ...
    'inspector/frontend/**/*.{ts,js,svelte}': (filenames) => {
        const abs = filenames
            ...
            .filter((f) => {
                ...
                // Skip generated trees: the Pydantic→TS schemas codegen and the
                // Paraglide JS i18n output. eslint --fix mangles generated files
                // (strips banners / reformats) and both dirs are regenerated by
                // their own toolchains.
                if (norm.startsWith('src/lib/types/generated/')) return false;
                return !norm.startsWith('src/lib/paraglide/');
            });
