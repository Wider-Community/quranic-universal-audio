...
- **Shared schemas live in `qua_shared/schemas/`** (Pydantic v2), split into `bucket/` (external bucket-resident JSON), `wire/` (HTTP request/response), and `config/` (SQLite-backed state/config) by where the shape lives. `extra=` is **direction-aware**: `extra="forbid"` is the default everywhere; `bucket/` artefacts add a `strip_and_warn` pre-validator (legacy tolerance — dead-field INFO strip + unknown-field WARNING); `extra="allow"` is used only when a forward-compat round-trip fixture proves an unknown field rides through unchanged. Both the offline extraction pipeline and the Inspector save flow MUST round-trip the per-reciter artefact shapes (`detailed.json` segs, `edit_history.jsonl` batches/ops, `edit_history_peaks.jsonl` records) through these models — never construct dict literals at the writer site. The slim FE-facing subset is re-exported at `qua_shared/schemas/fe_types.py`. → [`schemas.md`](docs/reference/schemas.md)
