...
| `inspector/` | The Flask app (tree below); `inspector/scripts/` holds inspector-coupled ops CLIs. |
| `scripts/` | Repo-wide ops CLIs — only `upload_inspector.py` (deploys the whole repo). |
...
- **Scripts placed by coupling, flat within a home** — operational CLIs go where their coupling points: import `services.*` / touch the inspector bucket → `inspector/scripts/` (flat; completed one-shot schema migrations frozen under `inspector/scripts/migrations/`); invoked only by a workflow → `.github/scripts/`; unit of work is the whole repo → root `scripts/`. Code *imported* at runtime is a package (`qua_shared`/`qua_jobs`), not a script. Don't add `role/` subfolders — the name-prefix groups them (KISS). Full decision rule + catalogue: [`inspector/scripts/README.md`](inspector/scripts/README.md).
