...
| **Run the app (any mode/worktree, conflict-free)** | `python scripts/devenv/launch.py` (the `/launch` skill) — defaults to **prod-remote** (real data, works on Windows); `--mode dev-remote` for the dev bucket, `--mode dev` for a local backend. Free ports, isolated DB, readiness wait, `list`/`down`/`doctor`, `--smoke`. Prefer this over hand-running the two below. |
