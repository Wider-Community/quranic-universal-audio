...
- The graph is git-ignored (a local/CI build artifact). Rebuild on demand with `graphify update .` (AST, no API cost) or `/graphify` (full + semantic); CI rebuilds it on merge to main. There is no per-commit rebuild hook.
