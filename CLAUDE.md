...
- **The Inspector rule catalogue is generated, presentation policy is not** — `scripts/codegen/regen_tajweed_rules.py` pins quranic-phonemizer 2.14 and writes the producer-owned 45-rule names/summaries to `timestamps/data/rules.json`. Inspector grouping, colors, defaults, duration labels, and stacks remain explicit in `tajweed-rules.ts`; neither belongs in shard v12.
