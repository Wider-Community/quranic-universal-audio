...
| `GET /admin/reviews/<slug>` | General-drawer payload — base + current claim + claim history + transition timeline + `timestamps_job_ids`. Three small bounded queries; cheap enough to fetch eagerly on every drawer open. 404 on unknown slug. |
| `GET /admin/reviews/<slug>/validation` | Lazy validation category counts (wraps `validate_reciter_segments`). `has_data=False` when the bucket has no `detailed.json` yet (typical for fresh `awaiting_review`). Expensive — only fetched when the accordion expands. |
