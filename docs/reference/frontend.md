...
- **Teleprompter** (`LineAnimation`) — stop marks are split out of the reveal (`build-structure.ts` → `AnimWord.clean`/`waqf`) into a neutral `.ra-waqf` span that **never** takes the highlight; it lights (`.waqf-active`) only while a pause holds on its word.

Stop-sign detection is one shared util — `lib/utils/waqf.ts` (`splitWaqf`/`hasWaqf`), the surfaced marks being U+06D6–DC **excluding** U+06D9 (لا, "do not stop").
...
| `waqf.ts` | Quranic waqf (stop) marks — `splitWaqf`/`hasWaqf` (U+06D6–DC, excludes لا U+06D9). Shared by the analysis pause bridge + teleprompter stop-sign handling |
