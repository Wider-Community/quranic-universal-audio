...
**Column alignment** (`UnifiedDisplay`). The analysis letter row and phoneme row share ONE per-group column grid — `.mega-grid` declares one track per cell-group (`repeat(--col-count, max-content)`) and both rows re-expose those tracks via CSS `subgrid` — so every phoneme sits directly beneath its letter-group and the relationship reads from alignment, not only when co-highlighted. The alignment unit is the **group** (`RenderedGroup`: a `base` consonant + its pinned ḥaraka/tanwīn, or a long-vowel `[diacritic, carrier]` pair / implicit madd); `_bucketPhonemes` assigns each rendered phoneme to its owning group's `.phoneme-cluster` via the cells' `phonemeIndices` (the synthetic-base fallback, used only by cell-less test fixtures, has no mapping and drops its phonemes into the last cluster). A 1-letter→2-sound group simply widens its shared column; a long vowel's shared phoneme shows once. A **silent base letter reserves an empty cluster** beneath it (a "makes no sound" slot that keeps the columns aligned); a truly-dropped diacritic at waqf (and any cell with no own phoneme) likewise adds nothing to its column — except the carried-vowel cases (`madd_iwad`, `allah_dagger_alef`, idgham-shafawi/noon absorbed vowels), which co-light on their carrier's phoneme. Cross-word mergers (idgham / iltiqaa-kasra) belong to neither word's columns and are lifted to the between-word `crossword-bridge` tile. Hovering any cell tints its whole column (`.group-hover`, matched by `data-group-index`); the resting cue is proximity + alignment alone.
