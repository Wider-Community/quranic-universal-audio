...
**Column alignment** (`UnifiedDisplay`). The analysis letter row and phoneme row share ONE per-group column grid — `.mega-grid` declares one track per cell-group (`repeat(--col-count, max-content)`) and both rows re-expose those tracks via CSS `subgrid` — so every phoneme sits directly beneath its letter-group and the relationship reads from alignment, not only when co-highlighted. The alignment unit is the **group** (`RenderedGroup`: a `base` consonant + its pinned ḥaraka/tanwīn, or a long-vowel `[diacritic, carrier]` pair / implicit madd); `_bucketPhonemes` walks the phonemes in reading order and assigns each to its owning group's `.phoneme-cluster` via the cells' `phonemeIndices`; a render-only phone that no cell indexes — the qalqala echo `Q` — rides the **preceding** consonant's group so it stays beside its source instead of jumping to the word end (the synthetic-base test fixtures have no mapping, so their phones fall to the first group, order preserved). A 1-letter→2-sound group simply widens its shared column; a long vowel's shared phoneme shows once. A **silent base letter reserves an empty cluster** beneath it (a "makes no sound" slot that keeps the columns aligned); a truly-dropped diacritic at waqf (and any cell with no own phoneme) likewise adds nothing to its column — except the carried-vowel cases (`madd_iwad`, `allah_dagger_alef`, idgham-shafawi/noon absorbed vowels), which co-light on their carrier's phoneme. Cross-word mergers (idgham / iltiqaa-kasra) belong to neither word's columns and are lifted to the between-word `crossword-bridge` tile. Hovering any cell tints its whole column (`.group-hover`, matched by `data-group-index`); the resting cue is proximity + alignment alone.

**Row justification** (`UnifiedDisplay`). Wrapped word rows are justified so a multi-row verse reads as an aligned block: `.unified-display` is `text-align: justify` + `text-align-last: start`, so each interior row flushes BOTH container edges with even gaps while the last row and single-row verses stay natural. The justify atoms are `.word-unit`s (inline-flex) built by `groupUnits(rendered)`: a `block.bridge` always links a block to its predecessor, so bridge-linked words — and consecutive-idgham chains — share ONE unit (a bridged pair never splits across rows); a `block.pauseBridge` (the silence carrying the PRECEDING word's waqf) is appended as that word's unit's trailing `.pause-bridge`, so the stop cell is the row's flush anchor, never the next word's lead. The space the justify distributes is an explicit `{' '}` between units; `word-spacing` sets its non-justified minimum gap.
