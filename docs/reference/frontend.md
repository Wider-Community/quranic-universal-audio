...
- **Filmstrip** — `snap` mode freezes during silence (active cell de-highlights, no needle). The **scrolling** modes (`hybrid`/`tuner`) no longer freeze: the needle stays visible but **greys** (`--text-muted`) and scrolls **continuously** across the inter-cell gap for a between-verse pause (`scrollThroughGap` interpolates offset by elapsed-silence; `nextIntervalAfter` resolves the upcoming cell). Within-verse / leading / trailing silence holds. Inter-cell gaps are **silence-scaled**: `VerseCell.nextGapSec` (between-verse silence) maps to a per-cell gap from `filmstripGapPx` (floor) up to `filmstripGapPx × filmstripGapMaxScale` (6×), normalized per chapter; snap mode keeps the fixed floor.
