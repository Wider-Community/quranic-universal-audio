...
**Row layout** (`UnifiedDisplay`). Wrapped word rows are **centered with one uniform gap** so a multi-row verse reads as an aligned block: `.unified-display` is `display: flex; flex-wrap: wrap; justify-content: center` with a single flex `gap` (`--mega-line-gap`) that is identical between every unit and on every row — one consistent rhythm, no row can open a too-wide gap, and `center` keeps equal inset on both container sides. The layout atoms are `.word-unit`s (inline-flex) built by `groupUnits(rendered)`: a `block.bridge` (idgham / iltiqaa) and a `block.pauseBridge` (inter-word silence) are both **connectors** that pair a block with its predecessor into ONE unit with the connector tile between them — so a bridged pair (and consecutive-idgham chains) and a word+stop-cell+word triple each stay on one row, never splitting. A pause cell therefore sits **between** its two paired words (a symmetric nestle via its own `margin-inline`), not trailing one.
