...
`qua_shared/schemas/` is split into three subpackages by where the shape lives: `bucket/` (external bucket-resident JSON artefacts — `segment`, `edit_history`, `peaks_history`, `catalog`, `pipeline_meta`, `ts_validation`, `ts_job_record`, `playlist_map`, `ts_shard`, `segments_doc`), `wire/` (HTTP request/response — `admin_*`, `intake_requests`, `mark_ready`, `release`, `seg`, `timestamps`, `public`, `audio`, plus `_envelopes` `ErrorEnvelope`/`OkAck`), and `config/` (state+db blobs — `state`, `access`, `audit`, `capabilities`, `automation`, `activity_state`, `pending_requests`). The root keeps the full barrel (`__init__.py`), the slim FE codegen surface (`fe_types.py`), `_extras.py` (`strip_and_warn`), and `smoke.py`. `extra` policy: bucket artefacts use `extra="forbid"` + `strip_and_warn` (legacy tolerance — the dead-field sets shrink once a prod migration empties them); wire request/response + config use `extra="forbid"`; `extra="allow"` only where a forward-compat fixture proves an unknown field rides through unchanged (e.g. `ts_validation` provenance). Codegen is unchanged in mechanism — `scripts/codegen/regen_fe_types.py` over `fe_types.py` → `inspector/frontend/src/lib/types/generated/schemas.ts`, gated by CI `schema-codegen-check`.
...
`inspector/services/storage/bucket_audit.py` is the Flask-free engine that walks one `reciters/<slug>/` folder and parses every artefact through the canonical schemas, surfacing the legacy-field INFO strips + unknown-field WARNINGs the `_extras` pre-validator emits. It backs three callers: the per-reciter CLI `scripts/bucket/audit_bucket_reciter.py`, the whole-bucket gate `scripts/diagnostics/validate_bucket.py` (per-reciter artefacts + DB-catalog rebuild + audio-manifest sidecars; exits non-zero on any hard error, `--strict` also fails on unknown fields), and `sample_validation` wired into `/healthz?deep=1`. The nightly `.github/workflows/bucket-validate.yml` runs the whole-bucket validator so external-file drift is caught in CI rather than mid-request.

### Planned: dead-set-emptying prod migration

The bucket artefact models tolerate known-legacy fields via `strip_and_warn` against per-model dead-field sets (e.g. `_OP_DEAD_FIELDS` / `_BATCH_DEAD_FIELDS` in `bucket/edit_history.py`). A gated future phase will run a one-shot prod-data migration that strips those dead fields in place so the dead-sets can shrink toward empty. It is **not done yet** — until then the tolerance stays and legacy records read through unchanged.
