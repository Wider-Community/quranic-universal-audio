...
| `request_views` | per-admin "viewed" marks for the Requests tab, `(request_id, hf_user_id)` PK (mirrors `activity_dismissals`); a pending request is "unviewed" until a row exists. Drives the per-admin unviewed badge. Migration `0004` | `repo_requests` |
| `review_views` | per-admin "viewed" marks for the Reviews tab → marked-ready dot, `(slug, hf_user_id)` PK with **upserted** `viewed_at` (not insert-or-ignore — latest view wins). A marked-ready row is "unviewed" when `viewed_at IS NULL OR viewed_at < claims.marked_ready_at`; cycle-safe by construction (send-back → re-mark-ready re-arms the dot). Migration `0005` | `repo_review_views` |
...
| `repo_review_views` | `review_views` | `mark_viewed` (UPSERT on `(slug, hf_user_id)`), `viewed_at_for_user`, `count_unviewed_marked_ready_for_user` (JOIN against open `claims` with `marked_ready_at` predicate) | `dict[slug, viewed_at_iso]` / `int` |
