...
| `ts_verse_flags` | public Timestamps-tab verse-issue reports; one row per `(slug, verse_key, identity)` where identity is `hf_user_id` (signed in) XOR `anon_token` (anonymous, minted in browser localStorage). Two **partial** unique indexes split the per-identity uniqueness (NULLs compare distinct) — `ON CONFLICT` targets must echo the index `WHERE` predicate. Optional `comment`. Migration `0024` | `repo_ts_flags` |
...
| `repo_ts_flags` | `ts_verse_flags` | `verse_counts(slug)`, `list_for_verse(slug, verse_key)`, `upsert_flag(...) -> (row, created)` (per-identity ON CONFLICT), `delete_flag(...)` | plain dicts (the route assembles the `TsFlagComment` wire model) |
...
**Adding a migration:** create `NNNN_<desc>.sql` (next 4-digit number) in `migrations/`. Write raw DDL/DML only — do NOT add `BEGIN`/`COMMIT`/`PRAGMA user_version` (the runner wraps it). It applies on next boot. Current head: `0024_ts_verse_flags.sql`.
