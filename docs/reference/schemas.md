...
- **Positional tuples → `RootModel[tuple[...]]`.** A shard word is a flat 5-slot tuple `[word_idx, start_ms, end_ms, letters[], phones[]]`, modelled as `TsShardWord(RootModel[tuple[int, int, int, list[LetterTiming], list[PhoneTiming]]])` (`bucket/ts_shard.py`) so the FE gets a positional TS tuple, not an object. **json2ts limitation:** Pydantic v2 emits JSON-Schema-2020-12 `prefixItems` for tuples (correctly typed), but the pinned json2ts (15.0.4) only understands the Draft-07 `items: [...]` form and renders tuple ELEMENTS as `unknown`. The models are correct; making the FE tuple typed is a codegen-pipeline fix (down-convert `prefixItems` → `items`), deferred. Same caveat applies to every `t: tuple[int, int]` span and `peaks: list[tuple[float, float]]`. The shard word is a 5-**or**-6 tuple (the 6th slot is `cells`); its slot-by-slot meaning is [shards.md](shards.md).
- **Enums that appear only inside a positional tuple → a parallel object model as the codegen vehicle.** A shard cell row's `role`/`status`/`rules` are the `CellRole`/`CellStatus`/`TajweedRule` enums (`bucket/cell_vocab.py`, `bucket/tajweed_vocab.py` — mirrors of the phonemizer's vocabulary, guarded by `qua_shared/tests/test_cell_vocab_parity.py`, since the phonemizer must stay off the Inspector import path). Because they are referenced ONLY inside the positional `CellTiming` tuple, json2ts renders them as `unknown` and never emits the named unions. To give the FE importable string unions, `bucket/ts_shard.py` also defines `TsShardCell` — a plain object model over the same fields — re-exported by `fe_types.py` purely as the codegen vehicle; it is never validated against real shard data. The wire cell stays positional and its rule slots stay open-form `str`, so an unknown/future tag rides through the byte-pass-through read unchanged; the FE reads the row by name through `parseShardCell` (`lib/types/ts-client.ts`), the mirror of `qua_shared/ts_shard_cells.parse_cell`. The emitted `TajweedRule` union is what the FE rule registry (`tabs/timestamps/utils/tajweed-rules.ts`) types its keys against via `satisfies`, so a phonemizer rename/addition is a **compile error**, never a silently-dropped underline. Cell-row slots and the schema versions that moved them: [shards.md](shards.md).
