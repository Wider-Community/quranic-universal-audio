...
The seg/ts/public/audio wire shapes are now modelled in `wire/` and codegen'd, replacing the hand-mirrored `api.ts` / `domain.ts` / `public-state.ts` (those still exist for now; deletion is a later phase).
