...
- **Enums that appear only inside a positional tuple → a parallel object model as the codegen vehicle.** Each shard cell row's `role`/`status` are the `CellRole`/`CellStatus` enums (`bucket/cell_vocab.py` — a mirror of the phonemizer's vocabulary, guarded by `qua_shared/tests/test_cell_vocab_parity.py` since the phonemizer must stay off the Inspector import path). Because they are referenced ONLY inside the positional `CellTiming` tuple, json2ts renders them as `unknown` and never emits the named unions. To give the FE importable `CellRole`/`CellStatus` string unions, `bucket/ts_shard.py` also defines `TsShardCell` — a plain object model over the same fields — re-exported by `fe_types.py` purely as the codegen vehicle. The wire cell stays positional; the FE reads it by name through `parseShardCell` (`lib/types/ts-client.ts`), the mirror of `qua_shared/ts_shard_cells.parse_cell`. The cell `tag` / `secondary_tags` / `phoneme_rule_tags` carry the tajweed **rule keys**: on the wire the positional `CellTiming` keeps them **open-form `str`** (so an unknown/future tag rides through the byte-pass-through read unchanged), but the `TsShardCell` codegen vehicle types those slots as the `TajweedRule` enum (`bucket/tajweed_vocab.py` — a mirror of the phonemizer's `TajweedRule`, guarded by `test_cell_vocab_parity.py` exactly like role/status). That makes the codegen emit a `TajweedRule` string union, which the FE rule registry (`tabs/timestamps/utils/tajweed-rules.ts`) types its keys against via `satisfies` — so a phonemizer rename/addition is a **compile error**, never a silently-dropped underline. A `FeSynthesizedTag` allow-list covers the FE/SDK-synthesized tags (`izhar_*`, `iqlab_silent_noon`, `iltiqaa`/`iltiqaa_kasra`, `madd_iwad`, `allah_dagger_alef`), and a `PIPELINE_ONLY_TAGS` set + a compile-time `Exclude` assertion force every producer rule to be classified (rendered or pipeline-only). `SEGMENT_SCHEMA_VERSION` bumped **v6→v7** when that vocabulary expanded (madd subtypes, plain ghunnah, ikhfaa/idgham shafawi for per-rule colour badges); the row shape is unchanged, so the bump only triggers a re-stamp. Later bumps add OPTIONAL trailing cell slots (additive row shape): **v8** `phoneme_rule_tags` (slot 7 — a per-phoneme rule list, set only on muqattaat cells) and **v9** `secondary_tags` (slot 8 — the non-winning colourable rules a cell also carries, in practice `['tafkheem']` on a heavy madd/qalqala cell, so the FE stacks a second underline; slot 7 is padded `None` when only slot 8 is present). Both are read defensively by `parse_cell` / `parseShardCell` and tolerated absent on older shards.
