...
| `GMAIL` / `GMAIL_PASS` | Space secret (optional) | Gmail account + app password for the email-notifications sender (`services/email/`). **Both unset → no email is sent; the sender logs the rendered message instead** (dev exercises the flow without secrets). `SMTP_HOST`/`SMTP_PORT` default to Gmail STARTTLS. |
