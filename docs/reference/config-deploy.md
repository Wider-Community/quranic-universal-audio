...
| `INSPECTOR_DB_SYNC` | unset (`1`) | `app.py`, `services/db/sync.py` | `0` disarms per-commit bucket DB write-back at boot (read-only). Used by `launch.py --mode prod` so a local process reading the prod bucket can never sync the full-file DB back over production. Boot still pulls the DB; nothing is ever pushed. |
