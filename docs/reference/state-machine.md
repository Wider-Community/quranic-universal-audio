...
| `last_job_finished_at` | `delivery_states` | datetime \| null (ISO) | Stamped on a timestamps job's terminal state — success (alongside publish→released) and failure (no state change) — by `state.mark_timestamps_job_finished` (no transition; job bookkeeping). **Not on `ReciterRow`** (`repo_state._assemble` reads named columns only — no schema/codegen change). Drives the Reviews-tab notification dot for job completions: a row is "unread" when `max(marked_ready_at, last_job_finished_at) > review_views.viewed_at` (see `repo_review_views.count_unviewed_for_user`). |
