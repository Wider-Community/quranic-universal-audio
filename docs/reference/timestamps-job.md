...
The bucket is recitation truth (every segment raw); a single canonical take per verse is a **pure projection** consumers apply (releases, the HF dataset, and conceptually the default per-verse clip). `project_segment_shard(shard, conf_by_span=)` in `qua_shared/timestamps_dedup.py` returns `{ref: {words, verse_start_ms, verse_end_ms}}`:
...
Words are **not** deduped inside a pass — only redundant material *after* the verse was already complete. This is tighter than the historical run-collapse: published word-lists get cleaner for ~187 verses (124 trailing-trim + 63 redundant-redo) across the 10 released reciters; interleaved loopbacks (71 verses) are the dropped non-canonical occasions, retained in the bucket for the filmstrip. The TS-tab read path serves segments raw — the same `project_segment_shard` shape feeds the release/dataset adapters, the dashboard now-reciting board, and the FE's per-verse default clip, so they cannot drift at the *occasion-selection* layer.

The FE mirror (`occasion-dedup.ts::canonicalClip`) exposes one deliberate divergence at the *trim* sub-step: the **Timestamps tab opts into `keepAllTakes`** (`assembleVerseFromShard(..., {keepAllTakes:true})` in `TimestampsTab.svelte`), retaining the canonical occasion verbatim so a verse recited several times **consecutively** renders its words as inline repeats and the clip span covers every take (playback rides through all of them). It's a faithfulness view, not the dataset — releases, the HF adapters, and the dashboard board keep the trimmed one-take clip. Interleaved / multi-verse loopbacks stay canonical-only on both paths (they're dropped non-canonical occasions, not a within-occasion trim).
...
- Format: `qua_shared/timestamps_shards.py` (`build_segment_shards`, `gzip_shard`), `qua_shared/timestamps_dedup.py` (`build_raw_v2`, `project_segment_shard`, `confidence_by_span`), `qua_shared/timestamps_pipeline.py` (`_normalize_from_results`); tests `qua_shared/tests/test_timestamps_segment_shards.py`, `test_segment_shard_dedup.py`, `test_timestamps_dedup.py`
