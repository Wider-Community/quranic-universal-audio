...
A leading/trailing repeat (the reciter restarts the verse) is deduped; a within-pass backward **lookback** (a jump back to a non-first word) is recitation truth and kept verbatim. Words are **not** deduped inside a pass — only redundant material *after* the verse was already complete. This is tighter than the historical run-collapse: published word-lists get cleaner for ~187 verses (124 trailing-trim + 63 redundant-redo) across the 10 released reciters; interleaved loopbacks (71 verses) are the dropped non-canonical occasions, retained in the bucket for the filmstrip. The TS-tab read path serves segments raw — the same `project_segment_shard` shape feeds the release/dataset adapters, the dashboard now-reciting board, and the FE's per-verse default clip (`occasion-dedup.ts` mirror), so they cannot drift at the *occasion-selection* layer.
