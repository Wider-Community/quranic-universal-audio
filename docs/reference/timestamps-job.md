...
| Env | Effect |
|---|---|
| `MFA_MODEL_PATH`, `MFA_DICTIONARY_PATH` | The MFA acoustic model + dictionary (launcher sets both to `/aux/mfa-runtime/…`). When unset, both derive from `MFA_APP_PATH`'s directory (legacy fallback, kept one release so a launcher/staging mismatch can't strand prod). Missing files → exit 3. |
