...
- **Drop-up** (footer Report button) — pick a category, in order `timing` →
  `tajweed` → `phonemes` → `audio` → `other`. `audio` / `other` open an inline
  comment composer (verse-level); `timing` / `tajweed` / `phonemes` enter report
  mode. `mapping` is a valid backend category but not surfaced — kept as
...
| `target_kind` + `word_index` / `source_letter_index` / `cell_index` / `phoneme_flat_index` / `share_group` | the flexible target descriptor (`verse\|word\|cell\|phoneme\|column\|cell_group`). `phonemes` reports only target `phoneme` — a normal phoneme has `phoneme_flat_index ≥ 0` (word-local indexable index); a cross-word merger/bridge phoneme has `phoneme_flat_index = -1` and keys on `word_index` (the word it renders before — see Bridge phonemes) |
| `target_key` | canonical descriptor string (`kind:wi:sli:ci:pi:sg`, **plus `:subtype` for tajweed**) the per-identity unique index keys on — built in `repo_ts_reports.target_key()`. Timing/phonemes are subtype/axis-free → one report per target+identity |
...
| `comment` | mandatory for `audio`/`mapping`/`other` + every tajweed; optional for timing; never for phonemes |
...
**public** — returned to every viewer. `tajweed` + `phonemes` flags are
...
| File | What |
|---|---|
| `inspector/services/db/repo_ts_reports.py` | `create` (upsert), `create_many` (batch, one identity/verse/txn), `resolve` (per id), `resolve_group` (timing/phoneme word-group, all identities), `delete` (soft), `verse_counts` + `list_for_verse` (both take the viewer ctx + `_visibility_filter`), `my_reports`, `list_open_for_recheck`, `word_group_key` |
...
| File | What |
|---|---|
| `stores/report-mode.ts` | the mode state machine (`inactive` / `timing` / `tajweed` + subtype fixed at entry / `phonemes`), `staged` Map keyed by cell, `focusedCellKey`, `reportContext`; `enterTiming` / `enterTajweed` pause + force letters-only (snapshot/restore `showLetters`/`showPhonemes`), tajweed additionally `forceAllTajweedEnabled`; `enterPhonemes` instead forces `showPhonemes` ON and leaves `showLetters` at the user's setting (the inverse). Seed own flags by category+subtype; a `StagedPhoneme` carries just the target (no comment/subtype) and is always complete; `focusCell`/`isStagedComplete` auto-discard an incomplete cell on focus-move; `exitReportMode` restores display + tajweed snapshots, `exitLoop`, clears |
...
| `components/UnifiedDisplay.svelte` | stamps `data-cell-index`/`-source-letter-index`/`-share-group`/`-has-tj`/`-tj-tags`/`-phoneme-flat-index`; a delegated capture-phase click that STAGES via `focusCell` (auto-discard) instead of seeking. **Only timing loops/seeks the cell** — tajweed/phonemes stage only, leaving play/pause + the whole-verse loop untouched. Phonemes mode is a multi-select **toggle** (re-clicking a flagged phoneme removes it) and inerts the letter/diacritic cells (NOT the block, which contains the phoneme spans), leaving only `.mega-phoneme` spans live. Reactive `report-*` passes (`report-dim`+`report-inert` spotlight, staged/focused/public flags; mode-scoped `report-timing`/`report-tajweed`/`report-phonemes`). `data-has-tj='1'` ⇐ the cell carries a `ruleTag` (pickable rule) OR a badge/silent name |
| `components/TimestampsFooterReport.svelte` + `report/ReportMenu.svelte` + `ReportComposer.svelte` | the drop-up: category list, inline audio/other composer (fixed-height field so it never reflows the drop-up), and `onenterMode('timing'\|'tajweed'\|'phonemes')` → report mode (timing/phonemes enter directly, tajweed expands its subtype rows) |
| `components/report/ReportControlStrip.svelte` | the strip that replaces the waveform — header (title + static subtype label, no toggle), Cancel + Submit, and ONE inline row per staged cell (`label · subtype/rule control · comment · ✕`). Phonemes collapse to ONE row per word, each flagged phoneme a compact removable chip (glyph from the clicked span / seeded `snapshot.chars`) — no control, no comment |
...
- **Display forcing differs by mode.** `timing` + `tajweed` force letters-only
  (phonemes are not a target there) — both footer toggles are locked. `phonemes`
  mode forces the phoneme row ON and leaves the letters row at the user's setting,
  so the letters toggle stays operable while the phonemes toggle is locked on.
...
- `inspector/tests/services/test_ts_target_snapshot.py` — phoneme + bridge-phoneme
  resolution and per-category staleness.
- `qua_shared/tests/test_ts_reports_wire.py` — per-category validators (incl.
  phonemes), batch models, `selected_rule_tags` gate, single/batch validation parity.
