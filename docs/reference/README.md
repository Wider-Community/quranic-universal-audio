...
| [timestamps-job.md](timestamps-job.md) | Timestamps subsystem — where shards are written (raw segments, byte pass-through read), consumer-side completion-based dedup, owner preview, the one-off reshape migration, the admin-triggered HF generation job (prebuilt public job image → in-container MFA, alignment-only → per-reciter `reciters/<slug>/jobs/ts/<id>.json` records), and the verse-level `ts_validation.json` accordion. |
| [shards.md](shards.md) | **The timestamp shard reference.** Native v13 connected readings, schema-2 phonemizer documents, timing sidecars, identity ownership, generation, strict audit, and atomic cutover. |
| [native-renderer-audit.md](native-renderer-audit.md) | Full-corpus evidence defining the producer/package/Inspector ownership boundary after the temporary v11 proof adapter was removed. |
| [ts-reports.md](ts-reports.md) | Timestamps reader reporting — the `ts_reports` table (categorized, cell-addressable, soft-delete, wrong-rule tags), word-grouped timing vs per-cell tajweed vs per-gap silence (pause boundary/waṣl/missed), batch create + group resolve + staleness + silence auto-resolve-on-regen, and the FE drop-up + in-grid report mode (cell + pause-gap flagging, tajweed spotlight, missed-pause slots, persisted public flags). |
...
| [editions.md](editions.md) | Multi-riwayah support — the four editions and their three slug vocabularies, the `qua-domain` dependency and its Hafs refusal, what is projected vs. matched against Hafs, the per-surface changes (Segments script/font, validation tables, Timestamps word profile), the word-profile shard, release/dataset tiers, and the verification fixtures. |
| [validation.md](validation.md) | Registry-backed validation engine, categories, persisted classifier fields, drift gates. |
| [frontend.md](frontend.md) | The Svelte 5 SPA — dashboard/timestamps/segments tabs, `lib/` cross-tab, stores, charts, peaks caching. |
| [theming.md](theming.md) | The light/dark theme system — the token contract (`tokens.css` dark + `theme-light.css`), flash-free init, the `themeStore` + toggle, the `canvas-theme.ts` resolver for 2D-canvas surfaces, the theme-conditional accent-derivation (analysis cells / waveform / teleprompter / footer accent), and the checklist for adding a color. |
| [i18n.md](i18n.md) | Frontend internationalization (Paraglide JS 2.x) — split-by-area message files + flat prefixed keys, the compile/build-order wiring, the locale rune + `dir/lang` plumbing, generated-output exclusions, the do/don't-translate boundary, verification. Strings-only today; the RTL layout flip is a later workflow ([`../planning/i18n.md`](../planning/i18n.md)). |
