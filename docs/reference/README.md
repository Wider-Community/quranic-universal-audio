...
| [timestamps-job.md](timestamps-job.md) | Timestamps subsystem — v2 occurrence-preserving `.json.gz` shard format, read-path dedup + `?full`, owner preview, the admin-triggered HF generation job (prebuilt public job image → in-container MFA → persist-audio/peaks → per-reciter `reciters/<slug>/jobs/ts/<id>.json` records), and the verse-level `ts_validation.json` accordion. |
