...
| `<bucket>/inspector-wip/<slug>/data/recitation_segments/<slug>/segments.json` | per-reciter, **in-flight** | **bucket mount** (server reads from `<MOUNT>/inspector-wip/<slug>/...`) | bucket mount (atomic write; mount flushes within 2–30 s) | Same file layout the existing save flow expects. Mount is mounted at `INSPECTOR_BUCKET_MOUNT`, exposed to the save flow as the data dir |
| `<bucket>/inspector-wip/<slug>/data/recitation_segments/<slug>/detailed.json` | per-reciter, **in-flight** | bucket mount | bucket mount | |
| `<bucket>/inspector-wip/<slug>/data/recitation_segments/<slug>/edit_history.jsonl` | per-reciter, **in-flight** | bucket mount | bucket mount | Append-only |
| `<bucket>/inspector-wip/<slug>/data/recitation_segments/<slug>/edit_history_peaks.jsonl` | per-reciter, **in-flight** | bucket mount | bucket mount | History panel waveform cache |
| `<bucket>/inspector-wip/<slug>/data/recitation_segments/<slug>/low_confidence_v2.json` | per-reciter, **in-flight** | bucket mount | n/a (pipeline-written, copied in on alignment-completed) | Read-only sidecar |
...
│   └── audit.jsonl                # append-only, one line per state event
...
| Event | Action |
|---|---|
| Alignment pipeline finishes | Pipeline writes outputs into `<bucket>/inspector-wip/<slug>/data/recitation_segments/<slug>/...`. Inspector backend (notified via webhook from the pipeline OR via state-machine transition `awaiting_alignment → awaiting_review`) marks state. |
...
| Publish (maintainer action) | HF Job snapshots `<bucket>/inspector-wip/<slug>/...` → `inspector/segments/<slug>/...` on the dataset. On success, bucket entry is moved to `<bucket>/_archive/<slug>/<published_at>/` (or deleted if archive policy is "delete"). State transitions `ready_for_merge → awaiting_timestamps`. |
...
- Deployed mode: `<INSPECTOR_BUCKET_MOUNT>/inspector-wip/<slug>` (per-slug subtree of the bucket mount)
...
| `INSPECTOR_BUCKET_MOUNT` | `/data/inspector-bucket` | unused | HF bucket mount point inside the Space container |
| `INSPECTOR_BUCKET_REPO` | `hetchyy/quranic-inspector-wip` (prod) / `hetchyy/quranic-inspector-wip-dev` (dev) | unused | HF bucket name for diagnostic/admin paths |
| `INSPECTOR_HF_TOKEN` | secret | unset | HF token with write scope on the bucket; used by mount + audit append |
...
| `INSPECTOR_GITHUB_DISPATCH_TOKEN` | secret | unset | Tiny GitHub PAT used to fire `repository_dispatch` events on `reciter.completed` (only for triggering GH Actions workflows that write to GitHub: `update-reciters.yml`, `release.yml`) |
...
`scripts/build_audio_catalog.py` consolidates 391 per-reciter manifests under `data/audio/<cat>/<src>/<slug>.json` into one `data/audio_catalog.json.gz`:
...
- Same source matrix. **Kept** — read path exists at `routes/peaks.py:82`, wired to History panel via `tabs/segments/utils/data/reciter-actions.ts:72` and `playback/preview.ts:209`.
...
- One-shot script `scripts/migrate_state_to_bucket.py` that walks current GitHub issues + open PRs + on-disk data and seeds `<bucket>/state/reciter_state.json`.
...
**In scope:**
- Inspector backend reads under-review/awaiting-review data from `<bucket>/inspector-wip/<slug>/...`.
...
**In scope:**
- Save flow points at `<INSPECTOR_BUCKET_MOUNT>/inspector-wip/<slug>/...` (via `data_dir.resolve(slug)` helper).
...
| 11-category validator cold (`validate_reciter_segments`) | **300–600 ms** | `services/validation/__init__.py:143`. Pure Python loops |
...
- **6–8 truly concurrent active users** stay under p95 1 s (vs v1's 4–6, slightly better because no GIL contention from github-fetch parsing).
