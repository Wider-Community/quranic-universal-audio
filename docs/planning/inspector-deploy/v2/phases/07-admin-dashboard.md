...
- [ ] On the next publish of an unlocked reciter, the publish flow honors `previous_dataset_state`: if it was `completed`, after `released` the row auto-transitions to `completed` + fires `reciter.dataset_published` (no re-gating); if it was `released`, stays at `released`
