...
- **Legacy bucket layout decommissioned via D20 Option B.** The `<bucket>/manifest.json.gz` + `<bucket>/segments/<slug>/<chapter>.json.gz` + `<bucket>/timestamps/<slug>/<chapter>.json.gz` layout is **load-bearing today** (universal aligner Space Preload mode at `.local/spaces/quranic_universal_aligner/src/preload/manifest_client.py` reads it directly; Inspector deployed timestamps tab consumes the same format). But the same content lives in v2 — `<bucket>/catalog/reciter_catalog.json` + `<bucket>/catalog/audio_manifest/<slug>.json` (sidecars) cover the metadata side, `<bucket>/published/<slug>/segments.json` covers segments (`matched_ref` derivable from key + word indices, `confidence` drops from the UI), and Phase 6 lands per-chapter `<bucket>/published/<slug>/timestamps/<chapter>.json`. Two parallel migration tracks defined in [D20](../inspector-deferred.md): Track A (aligner Preload, blocks on catalog promotion only), Track B (Inspector TS-tab frontend, blocks on Phase 6 + catalog promotion). After both land, Phase 11 drops the legacy layout + `scripts/lib/{segments,timestamps}_shards.py`.
