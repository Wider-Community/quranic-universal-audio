...
- **Legacy bucket layout decommissioned via D20 Option B — both tracks landed in Phase 1.**
  ...
  - **Bucket cleanup:** `<bucket>/manifest.json.gz`, `<bucket>/segments/<slug>/`, `<bucket>/timestamps/<slug>/` all deleted from `hetchyy/quranic-inspector-bucket-dev`. Bucket top level is now `access/`, `audit/`, `catalog/`, `published/`, `state/`, `wip/` only.
