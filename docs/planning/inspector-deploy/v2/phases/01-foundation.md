...
- **Catalog promotion — done.** `scripts/inspector_v2_seed/promote_catalog.py` reads `.local/dedup/reciter_catalog.json` + 864 `audio_manifest/<slug>.json` sidecars, validates through pydantic, raw-bytes-uploads. Bucket now carries the real catalog (422 reciters, 864 deliveries, `schema_version=2`) + 864 sidecars under `<bucket>/catalog/audio_manifest/`. Bulk audio probe is still completing for the remaining `by_ayah` deliveries — those rows will land with `bitrate_mode=unknown` / `sample_rate_hz=null` until re-probed; not blocking.
- **Legacy bucket layout decommissioned via D20 Option B — both tracks landed in Phase 1.**
  - **Bridge:** `scripts/inspector_v2_seed/convert_legacy_timestamps.py` walked `<bucket>/timestamps/<slug>/<chapter>.json.gz` for the 6 completed reciters and re-uploaded as ungzipped `<bucket>/published/<slug>/timestamps/<chapter>.json`. Phase 6's publish HF Job will keep writing to the same path; no further conversion needed.
  - **Track A (aligner Preload):** new Reciter → Mushaf → Source cascading dropdown UX (Reciter from `catalog.reciters[]`, Mushaf grouped on `(riwayah, style, recording_year, variant_label)`, Source listed when Mushaf has >1 delivery). No top-level riwayah filter — riwayah surfaces inside Mushaf labels. No auto-pick on source — user picks when >1, dropdown hides when only 1. Confidence badge dropped from card display. Data layer: `manifest_client.py` rewritten to read `<bucket>/catalog/reciter_catalog.json` + sidecar `<bucket>/catalog/audio_manifest/<slug>.json` + whole-file `<bucket>/published/<slug>/segments.json` + per-chapter `<bucket>/published/<slug>/timestamps/<chapter>.json`. Deployed to `hetchyy/quranic-universal-aligner-dev`.
  - **Track B (Inspector TS tab):** minimal data-layer-only refactor. New backend route `/api/static/catalog.json` serves the in-memory catalog snapshot. Frontend `ts_client.ts` swaps `loadManifest()` for `loadCatalog()`. Same flat reciter list UX as before (no Mushaf cascade in the TS tab). Shard fetches for the chapter-by-chapter timestamps still go through the public dataset for now; their v2-paths migration is Phase 6 work.
  - **Bucket cleanup:** done. 1,369 legacy files deleted (`manifest.json.gz` + 684 `segments/<slug>/<chapter>.json.gz` + 684 `timestamps/<slug>/<chapter>.json.gz`) via `scripts/inspector_v2_seed/drop_legacy_bucket.py --confirm`. Bucket top level is now `access/`, `audit/`, `catalog/`, `published/`, `state/`, `wip/` only — pure v2 layout, single source of truth.
  - **Phase 11 still owns:** `scripts/lib/segments_shards.py` + `scripts/lib/timestamps_shards.py` deletion (after the v1 source dependency in repo `data/` is also gone), and `inspector/services/ts_local.py` deletion (after local-mode shard model is fully unwired).
