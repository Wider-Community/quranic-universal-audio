...
### Validators packaging revisit (supersedes S2-D03)
- **User call, 2026-04-15**: "vendoring validators is not the right choice … it should be outside of repo." S2-D03 vendored `validators/` into `inspector/validators/` at SHA `fb889d7` as a Wave-2a expedient to scope the Docker build context to `inspector/`. The decision note already flagged the cost ("the two copies diverge freely from here") and no synchronisation mechanism exists. Stage 3 should move validators to a proper dependency relationship — candidates: (a) published PyPI package (`pip install quranic-validators`) installed via `requirements.txt`; (b) git submodule in `inspector/third-party/validators/` pinned to an upstream commit with a `make update-validators` task; (c) Dockerfile builds from repo-root context with a two-path COPY (`inspector/` + `validators/`) instead of vendoring. Decide in Stage 3 interview; whichever wins, `inspector/validators/` directory deletes.
