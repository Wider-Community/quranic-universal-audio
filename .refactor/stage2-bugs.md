...
| S2-B06 | Section 2 | All 8 remaining cycles dissolved in Wave 11a P4 via the register* pattern: (1) `data ↔ history/index`: inlined `renderEditHistoryPanel` in data.ts + registered `onSegReciterChange` in history/index.ts; (2) `data ↔ playback/index`: registered `stopSegAnimation` in data.ts; (3) `data ↔ waveform/index`: registered `_fetchChapterPeaksIfNeeded` in data.ts + registered data lookups in waveform/index.ts; (4) `waveform/index ↔ rendering`: registered `_getEditCanvas` in waveform/index.ts. All wired in segments/index.ts. `import/no-cycle` re-promoted to `error`; CYCLE_CEILING updated to 0. | TBD | 11a |
